...
export default function FormPhotos({ photos, setPhotos, photoLink, setPhotoLink }) {
    ...
    async function uploadFromDevice(e) {
        ...
        try {
            const res = await axios.post('/place/photo/upload-from-device', formData, {
                headers: {
                ...
                {photos.length > 0 &&
                    photos.map((item) => (
                        <div key={item} className="flex relative">
                            <img src={`${import.meta.env.VITE_API_DOMAIN}:${import.meta.env.VITE_PORT}/uploads/${item}`} className="rounded-2xl h-32" />
